...
class LinkList extends Component {
  ...
  _subscribeToNewVotes = () => {
    this.props.allLinksQuery.subscribeToMore({
      ...
      `,
      updateQuery: (previous, { subcriptionData }) => {
        const votedLinkIndex = previous.allLinks.findIndex(link => link.id === subcriptionData.data.Vote.node.link.id);
        const link = subcriptionData.data.Vote.node.link;
...
export default graphql(ALL_LINKS_QUERY, {
  ...
  options: (ownProps) => {
    ...
    const skip = isNewPage ? (page - 1) * LINKS_PER_PAGE : 0;
    const first = isNewPage ? LINKS_PER_PAGE : 100;
    const orderBy = isNewPage ? 'create_DESC' : null;
    return {
